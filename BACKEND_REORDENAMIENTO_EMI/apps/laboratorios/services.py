...
class InventoryAnalyticsService:
    ...
    def obtener_resumen_nacional(cls):
        from django.db.models import ExpressionWrapper, F, IntegerField, Max
        ...
        pct_malo_regular = (
            round((malos_regulares / total_equipos * 100), 2) if total_equipos > 0 else 0
        )
        ...
        for eq in equipos:
            ...
            sede = (
                ...
                if eq.laboratorio and eq.laboratorio.unidad_academica
                else "Sin Asignar"
            )
        ...
        for nombre in nombres_con_excedente:
            ...
            con_excedente = [
                (lab_id, d, r)
                for lab_id, (d, r) in filas.items()
                if d > r * 2 or (r == 0 and d > 0)
            ]
            con_deficit = [(lab_id, d, r) for lab_id, (d, r) in filas.items() if r - d > 0]
    ...
    def uso_equipos_de_laboratorio(cls, laboratorio_id):
        ...
        """
        total_practicas_lab = cls._total_practicas_laboratorio(laboratorio_id)
        equipos = Equipo.objects.filter(laboratorio_id=laboratorio_id).annotate(
            practicas_uso=Count("guias_que_requieren__guia", distinct=True)
        )
