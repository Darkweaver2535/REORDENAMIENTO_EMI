...
class Laboratorio(BaseModel):
    ...
    def clean(self):
        ...
        if self.parent_id is None:
            ...
            except Laboratorio.DoesNotExist as err:
                raise ValidationError({"parent": "El laboratorio padre no existe."}) from err
            ...
            if self.pk is not None:
                ...
                while cursor_id is not None:
                    ...
                    try:
                        cursor_id = Laboratorio.objects.values_list("parent_id", flat=True).get(
                            pk=cursor_id
                        )
                    except Laboratorio.DoesNotExist:
                        break
...
class UsoAcademico(BaseModel):
    laboratorio = models.ForeignKey(
        Laboratorio, on_delete=models.CASCADE, related_name="usos_academicos"
    )
...
class EquipoRequeridoPorGuia(BaseModel):
    ...
    def tiene_deficit(self):
        return (
            self.equipo is not None and self.equipo.cantidad_disponible() < self.cantidad_requerida
        )
