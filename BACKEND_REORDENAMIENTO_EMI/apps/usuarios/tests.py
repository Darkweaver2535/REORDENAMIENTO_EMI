...
def _ua(nombre="UA Test", codigo="UATST"):
    return UnidadAcademica.objects.create(nombre=nombre, ciudad="Ciudad Test", codigo=codigo, abreviacion=codigo, is_active=True)

def _lab(ua, nombre="Lab Test", campus="Campus Test"):
    return Laboratorio.objects.create(nombre=nombre, unidad_academica=ua, campus=campus, capacidad_estudiantes=20)

def _equipo(lab, codigo="COD-T001"):
    return Equipo.objects.create(nombre="Equipo Test", codigo_activo=codigo, laboratorio=lab, unidad_academica=lab.unidad_academica, cantidad_total=1, cantidad_buena=1, cantidad_regular=0, cantidad_mala=0)

def _usuario(carnet, rol, ua=None, password="test1234"):
    u = Usuario.objects.create(carnet_identidad=carnet, nombre_completo=f"Usuario {carnet}", rol=rol, unidad_academica=ua)
    u.set_password(password); u.save(update_fields=["password"]); return u

def _client_as(usuario):
    c = APIClient(); c.force_authenticate(user=usuario); return c
...
class LoginTests(TestCase):
    def setUp(self):
        ...
        self.ua = _ua(); self.admin = _usuario("ADMIN001", Usuario.Rol.ADMIN, self.ua)

    def _login(self, carnet, password="test1234", ip="127.0.0.1"):
        return self.client.post("/api/v1/auth/login/", {"carnet_identidad": carnet, "password": password}, format="json", REMOTE_ADDR=ip)

    def test_login_exitoso_devuelve_campos_requeridos(self):
        resp = self._login("ADMIN001")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        for campo in ("access_token", "refresh_token", "id", "carnet_identidad", "rol", "nombre_completo"):
...
class LoginThrottleTests(TestCase):
    ...
    def test_sexto_intento_retorna_429(self):
        ip = "10.99.99.1"
        for i in range(5):
            r = self.client.post("/api/v1/auth/login/", {"carnet_identidad": "X", "password": "x"}, format="json", REMOTE_ADDR=ip)
...
class ScopingInventarioTests(TestCase):
    ...
    def setUpTestData(cls):
        cls.ua1 = _ua("Sede A", "UA1ST"); cls.ua2 = _ua("Sede B", "UA2ST")
        cls.lab1 = _lab(cls.ua1, "Lab A1", "Campus A"); cls.lab2 = _lab(cls.ua2, "Lab B1", "Campus B")
        cls.eq1 = _equipo(cls.lab1, "EQ-A-T01"); cls.eq2 = _equipo(cls.lab2, "EQ-B-T01")
    ...
    def test_admin_ve_todos_los_equipos(self):
        ids = {e["id"] for e in _client_as(self.admin).get("/api/v1/laboratorios/equipos/").data["results"]}
        self.assertIn(self.eq1.id, ids); self.assertIn(self.eq2.id, ids)

    def test_encargado_ve_solo_equipos_de_su_ua(self):
        ids = {e["id"] for e in _client_as(self.encargado).get("/api/v1/laboratorios/equipos/").data["results"]}
        self.assertIn(self.eq1.id, ids); self.assertNotIn(self.eq2.id, ids)

    def test_encargado_sin_ua_no_ve_equipos(self):
        self.assertEqual(_client_as(self.encargado_sin_ua).get("/api/v1/laboratorios/equipos/").data["count"], 0)

    def test_estudiante_no_ve_equipos(self):
        self.assertEqual(_client_as(self.estudiante).get("/api/v1/laboratorios/equipos/").data["count"], 0)

    def test_admin_ve_todos_los_laboratorios(self):
        ids = {l["id"] for l in _client_as(self.admin).get("/api/v1/laboratorios/").data["results"]}
        self.assertIn(self.lab1.id, ids); self.assertIn(self.lab2.id, ids)

    def test_encargado_ve_solo_labs_de_su_ua(self):
        ids = {l["id"] for l in _client_as(self.encargado).get("/api/v1/laboratorios/").data["results"]}
        self.assertIn(self.lab1.id, ids); self.assertNotIn(self.lab2.id, ids)
    ...
    def test_encargado_ve_reordenamientos_de_su_ua(self):
        reo = Reordenamiento.objects.create(tipo_movimiento=Reordenamiento.TipoMovimiento.REASIGNACION_DEFINITIVA, equipo=self.eq1, laboratorio_origen=self.lab1, laboratorio_destino=self.lab2, cantidad_trasladada=1, estado=Reordenamiento.Estado.APROBADO)
        ids = {r["id"] for r in _client_as(self.encargado).get("/api/v1/reordenamientos/").data["results"]}
        self.assertIn(reo.id, ids)

    def test_encargado_no_ve_reordenamientos_de_otra_ua(self):
        eq2b = _equipo(self.lab2, "EQ-B-T02")
        reo = Reordenamiento.objects.create(tipo_movimiento=Reordenamiento.TipoMovimiento.REASIGNACION_DEFINITIVA, equipo=eq2b, laboratorio_origen=self.lab2, laboratorio_destino=self.lab2, cantidad_trasladada=1, estado=Reordenamiento.Estado.APROBADO)
        ids = {r["id"] for r in _client_as(self.encargado).get("/api/v1/reordenamientos/").data["results"]}
...
class DashboardRegressionTests(TestCase):
    @classmethod
    def setUpTestData(cls):
        ua = _ua("UA Dash", "UADSH"); cls.admin = _usuario("ADM_DASH", Usuario.Rol.ADMIN, ua)
    ...
    def test_dashboard_contiene_campos_esperados(self):
        ...
        for campo in ("total_guias_publicadas", "total_equipos", "laboratorios_activos", "reordenamientos_pendientes"):
            self.assertIn(campo, resp.data, f"Falta: {campo}")
